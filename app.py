...
@app.route("/backend/problems/<id>/my_rating", methods=["GET"])
@token_required
def get_my_rating(current_user, id):
    mysql = MySQL().connect(mysql_ip, mysql_db)

    mysql.query("select rating from problem_rating where user_id = %s and problem_id = %s", (current_user.id, id))
